...
;timers.c,21 :: 		if((battery_chosen_old == 1 || battery_chosen_old == 2)&& ACCUMULATOR_SENSE)
MOVW	R0, #lo_addr(_battery_chosen_old+0)
MOVT	R0, #hi_addr(_battery_chosen_old+0)
LDRSH	R0, [R0, #0]
CMP	R0, #1
IT	EQ
BEQ	L__Timer2_interrupt23
MOVW	R0, #lo_addr(_battery_chosen_old+0)
MOVT	R0, #hi_addr(_battery_chosen_old+0)
LDRSH	R0, [R0, #0]
CMP	R0, #2
IT	EQ
BEQ	L__Timer2_interrupt22
IT	AL
BAL	L_Timer2_interrupt4
L__Timer2_interrupt23:
...
L__Timer2_interrupt20:
;timers.c,23 :: 		if(duzinaSignala2 > dutyy){
...
L_Timer2_interrupt5:
;timers.c,26 :: 		else if(duzinaSignala2 < dutyy){
MOVW	R0, #lo_addr(_dutyy+0)
MOVT	R0, #hi_addr(_dutyy+0)
LDRSH	R1, [R0, #0]
MOVW	R0, #lo_addr(_duzinaSignala2+0)
...
L_Timer2_interrupt7:
L_Timer2_interrupt6:
;timers.c,30 :: 		pwm_percent =  (double)(((double)(((double)duzinaSignala2) * ((double)100))) / ((double)2000));
MOVW	R0, #lo_addr(_duzinaSignala2+0)
MOVT	R0, #hi_addr(_duzinaSignala2+0)
VLDR	#1, S0, [R0, #0]
VCVT.F32	#1, S1, S0
...
VSTR	#1, S1, [R0, #0]
;timers.c,31 :: 		pwm_duty = ((unsigned int)((double)(((double)(((double)pwm_ratio) * pwm_percent)) / ((double)100))));
...
STRH	R1, [R0, #0]
;timers.c,32 :: 		PWM_TIM3_Set_Duty(pwm_duty, _PWM_NON_INVERTED, _PWM_CHANNEL2);
UXTH	R0, R1
MOVS	R2, #1
MOVS	R1, #0
...
UXTH	R0, R0
STRH	R0, [R1, #0]
;timers.c,59 :: 		if(battry_percentage_check_counter >= 80){
CMP	R0, #80
IT	CC
BCC	L_Timer4_interrupt8
;timers.c,60 :: 		battry_percentage_check = 1;
MOVS	R1, #1
SXTB	R1, R1
MOVW	R0, #lo_addr(_battry_percentage_check+0)
MOVT	R0, #hi_addr(_battry_percentage_check+0)
STR	R1, [R0, #0]
;timers.c,61 :: 		battry_percentage_check_counter = 0;
MOVS	R1, #0
...
;timers.c,62 :: 		}
L_Timer4_interrupt8:
;timers.c,64 :: 		if(buzzerCounter >= 10000){
...
;timers.c,66 :: 		}
L_Timer4_interrupt9:
;timers.c,68 :: 		if(battery_change){
MOVW	R1, #lo_addr(_battery_change+0)
MOVT	R1, #hi_addr(_battery_change+0)
LDR	R0, [R1, #0]
CMP	R0, #0
IT	EQ
BEQ	L_Timer4_interrupt10
;timers.c,69 :: 		battery_change_count++;
...
SXTH	R0, R0
STRH	R0, [R1, #0]
;timers.c,70 :: 		if(battery_change_count> 100){
CMP	R0, #100
IT	LE
BLE	L_Timer4_interrupt11
;timers.c,71 :: 		battery_change_count = 0;
MOVS	R1, #0
SXTH	R1, R1
...
;timers.c,75 :: 		if(bitStart3SecondsCount){
...
UXTH	R0, R0
STRH	R0, [R1, #0]
;timers.c,77 :: 		if(delay3secndsCount >= 60){
CMP	R0, #60
IT	CC
BCC	L_Timer4_interrupt13
;timers.c,78 :: 		bitDelay3Seconds = 0;
MOVS	R1, #0
SXTB	R1, R1
MOVW	R0, #lo_addr(_bitDelay3Seconds+0)
MOVT	R0, #hi_addr(_bitDelay3Seconds+0)
STR	R1, [R0, #0]
;timers.c,79 :: 		bitStart3SecondsCount = 0;
MOVW	R0, #lo_addr(_bitStart3SecondsCount+0)
MOVT	R0, #hi_addr(_bitStart3SecondsCount+0)
STR	R1, [R0, #0]
;timers.c,80 :: 		delay3secndsCount = 0;
MOVS	R1, #0
...
;timers.c,84 :: 		if(bitControlTaken){
...
SXTH	R0, R0
STRH	R0, [R1, #0]
;timers.c,86 :: 		if(controlTakenCount>60){
CMP	R0, #60
IT	LE
BLE	L_Timer4_interrupt15
;timers.c,87 :: 		controlTakenCount = 0;
MOVS	R1, #0
SXTH	R1, R1
MOVW	R0, #lo_addr(_controlTakenCount+0)
MOVT	R0, #hi_addr(_controlTakenCount+0)
STRH	R1, [R0, #0]
;timers.c,88 :: 		bitControlTaken = 0;
MOVS	R1, #0
SXTB	R1, R1
MOVW	R0, #lo_addr(_bitControlTaken+0)
MOVT	R0, #hi_addr(_bitControlTaken+0)
STR	R1, [R0, #0]
;timers.c,89 :: 		control_taken = 0;
...
;timers.c,93 :: 		countJoystickConnection++;
...
UXTH	R0, R0
STRH	R0, [R1, #0]
;timers.c,94 :: 		if(countJoystickConnection > 60){
CMP	R0, #60
IT	LS
BLS	L_Timer4_interrupt16
;timers.c,95 :: 		countJoystickConnection = 60;
MOVS	R1, #60
MOVW	R0, #lo_addr(_countJoystickConnection+0)
MOVT	R0, #hi_addr(_countJoystickConnection+0)
STRH	R1, [R0, #0]
;timers.c,96 :: 		bitCheckJoystick = 1;
MOVS	R1, #1
SXTB	R1, R1
...
;timers.c,100 :: 		couterJoystickDisplay++;
...
UXTH	R0, R0
STRH	R0, [R1, #0]
;timers.c,101 :: 		if(couterJoystickDisplay == 1){
CMP	R0, #1
IT	NE
BNE	L_Timer4_interrupt17
;timers.c,102 :: 		bitCheckStatusDisplay = 1;
MOVS	R1, #1
SXTB	R1, R1
